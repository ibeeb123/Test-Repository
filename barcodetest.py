...
cap.set(cv2.CAP_PROP_FRAME_WIDTH,640)					# 프레임 설정
...
while True:												# 비디오에서 이미지를 한 장씩 가져옴	
	...
	break_point = False									# 다중 반복문을 나오기위한 변수 설정
	...
	if success:
		for code in pyzbar.decode(frame):				# 이미지에서 바코드를 찾고 디코딩
			...
			for i in range(0, len(data_list),1):		# 배열의 길이 만큼 반복 => 저장된 바코드를 찾기 위해
				...
				b = data_list[i]
				Splitb = b[0:13]						# 배열 값에서 '\n'추출 				

				if	Send_my_code == Splitb:          
					print("인식성공")

					if GPIO.input(button_pin) == GPIO.HIGH:				# 버튼이 눌렸을시											
					...
					if  check == str(880):				#조건 바꿔야 함(테스트용)						
						break_point = True																				
						break;
