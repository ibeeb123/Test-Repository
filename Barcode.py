...
button_pinA = 15                                        # 결제 화면에 데이터 전송하는 버튼
button_pinB = 14                                        # 데이터베이스에 데이터 전송하는 버튼
GPIO.setup(button_pinA, GPIO.IN, pull_up_down = GPIO.PUD_DOWN)
GPIO.setup(button_pinB, GPIO.IN, pull_up_down = GPIO.PUD_DOWN)
    ...
    if success:        
        for code in pyzbar.decode(frame):	# 이미지에서 바코드를 찾고 디코딩
            ...
            if bar == 0:
                ...
                if GPIO.input(button_pinA) == GPIO.HIGH:
                    ...
                    response = requests.post("http://gowoong.com:4000/api/test",data ={'data1': json.dumps(country),'data2' : json.dumps(manufactor),'data3' : json.dumps(item),'data4' : json.dumps(checknum),'data5' : "천안역점" })
                ...
                elif GPIO.input(button_pinB) == GPIO.HIGH:
                    country = my_code[0:3]
                    manufactor = my_code[3:7]
                    item = my_code[7:12]
                    checknum = str(my_code[12:13])
                    ...
                    #http://192.168.0.79:4000/api/test
                    response = requests.post("http://gowoong.com:4000/managerPage/registration",data ={'data1': json.dumps(country),'data2' : json.dumps(manufactor),'data3' : json.dumps(item),'data4' : json.dumps(checknum)})
                    server_response = int(response.status_code)
